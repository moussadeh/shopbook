...
import { Check, X, Clock, ExternalLink, Phone, Mail } from "lucide-react";
...
export default function AbonnementsView({ paiements }: { paiements: PaiementAboRow[] }) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-bold text-vert-foncee">Vérification des abonnements</h1>
          ...
          <div key={p.id} className="bg-white rounded-2xl border p-4 md:p-5">
            <div className="flex flex-col md:flex-row md:items-center gap-4">
              ...
              <div className="flex items-center gap-3 flex-1 min-w-0">
                <div className="w-11 h-11 rounded-full bg-vert-foncee text-white font-bold flex items-center justify-center shrink-0">
                  {p.initials}
                </div>
                <div className="min-w-0">
                  <p className="font-semibold text-gray-900 truncate">{p.boutique}</p>
                  <p className="text-xs text-muted-foreground truncate">{p.commercantNom}</p>
                  <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
                    <span className="flex items-center gap-1"><Phone size={11} />{p.telephone}</span>
                    <span className="hidden sm:flex items-center gap-1"><Mail size={11} />{p.email}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-5 md:gap-8 shrink-0">
                <div>
                  ...
                  <p className="text-sm font-semibold text-gray-800">{p.methode}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Montant</p>
                  <p className="text-sm font-bold text-gray-900">{p.montant.toLocaleString("fr-FR")} MRU</p>
                </div>
                <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${statutBadge[p.statut]}`}>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 mt-4 pt-4 border-t">
              <div className="flex items-center gap-3 text-xs text-muted-foreground">
                ...
                <a href={p.captureUrl} target="_blank" rel="noreferrer"
                  className="inline-flex items-center gap-1 font-semibold text-vert-foncee hover:underline">
              ...
              {p.statut === "EN_ATTENTE" && (
                <div className="flex items-center gap-2">
                  <Button variant="outline" onClick={() => setRejet(p)}
                    className="gap-1.5 text-red-600 border-red-200 hover:bg-red-50">
                    <X size={15} /> Rejeter
                  </Button>
                  <Button onClick={() => approuverPaiement(p.id)}
                    className="gap-1.5 bg-vert-foncee text-white hover:opacity-90">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              ...
              <Textarea id="motif" value={motif} onChange={(e) => setMotif(e.target.value)}
                placeholder="Ex: Paiement non reçu sur le compte Bankily." className="resize-none min-h-24" />
            </div>
            <div className="flex gap-3">
              <Button variant="outline" className="flex-1" onClick={() => { setRejet(null); setMotif(""); }}>
                Annuler
              </Button>
              <Button className="flex-1 bg-red-600 text-white hover:bg-red-700" onClick={confirmerRejet} disabled={!motif.trim()}>
                Confirmer le rejet
              </Button>
            </div>
          </div>
