...
import { Mail, Search, Reply, Check, Circle } from "lucide-react";
...
export default function MessagesView({ messages }: { messages: MessageRow[] }) {
  ...
  const [actif, setActif] = useState<MessageRow | null>(messages[0] ?? null);
  ...
  const ouvrir = (m: MessageRow) => {
    setActif(m);
    if (!m.lu) marquerLu(m.id, true); // marque lu à l'ouverture
  };
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-vert-foncee">Messages de contact</h1>
          ...
          <div className="divide-y overflow-y-auto max-h-[60vh]">
            {filtered.map((m) => (
              <button
                key={m.id}
                onClick={() => ouvrir(m)}
                className={`w-full text-left px-4 py-3 flex gap-3 hover:bg-gray-50/60 transition ${actif?.id === m.id ? "bg-green-50/50" : ""}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <p className={`text-sm truncate ${m.lu ? "font-medium text-gray-700" : "font-bold text-gray-900"}`}>{m.name}</p>
            ...
            <div className="space-y-5">
              <div className="flex items-start justify-between gap-4 border-b pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-gray-900">{actif.name}</p>
                    <p className="text-sm text-muted-foreground">{actif.email}</p>
                  </div>
                </div>
