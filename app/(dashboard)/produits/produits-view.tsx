...
import { buildProduitsStats } from "@/lib/donnes/produits";
...
const FILTRES = ["Tous", "Disponibles", "Indisponibles"];

export default function ProduitsView({ produits, stats }: { produits: ProduitRow[]; stats: ProduitsStats }) {
  ...
  const filtered = produits.filter((p) => {
    ...
    const matchFilter =
      filter === "Tous" ||
      (filter === "Disponibles" && p.disponible) ||
      (filter === "Indisponibles" && !p.disponible);
  ...
  return (
    <div className="px-4 md:px-6 py-5 space-y-5 max-w-7xl mx-auto">
      ...
      <div className="hidden md:block bg-white rounded-2xl border overflow-hidden">
        <ProduitsTable produits={paginated} onDetail={openDetail} onEdit={openEdit} onDelete={handleDelete} />
        {filtered.length === 0 && (
      ...
      <div className="md:hidden space-y-3">
        <ProduitsCards produits={paginated} onDetail={openDetail} onEdit={openEdit} onDelete={handleDelete} />
        {filtered.length === 0 && (
      ...
      <ProduitDetailDialog open={detailOpen} onOpenChange={setDetailOpen} produit={detail}
        onEdit={(p) => { setDetailOpen(false); openEdit(p); }} />
    </div>
  );
}
